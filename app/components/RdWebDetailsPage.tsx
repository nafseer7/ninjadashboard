...
const RdWebDetailsPage = ({ fileId }: { fileId: string }) => {
    const [fileDetails, setFileDetails] = useState<FileDetails | null>(null);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);
    const [searchQuery, setSearchQuery] = useState<string>("");
    const [sortField, setSortField] = useState<keyof RdWebUrl | null>(null);
    ...
    useEffect(() => {
        const fetchFileDetails = async () => {
            try {
                const response = await fetch(`/api/rdwebfiles/${fileId}`);
                if (!response.ok) {
                    throw new Error("Failed to fetch file details");
                }

                const data = await response.json();
                const rdWebUrls = data.file.rdWebUrls;

                // Fetch Moz API data for each URL
                const enrichedUrls = await Promise.all(
                    rdWebUrls.map(async (urlDetails: RdWebUrl) => {
                ...
                setFileDetails({ ...data.file, rdWebUrls: enrichedUrls });
            } catch (err) {
    ...
    const handleAccess = async (siteUrl: string, username: string, password: string) => {
        try {
            const response = await fetch("/api/login-rdweb", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
    ...
    const handleExportToCSV = () => {
        ...
        const csvRows = fileDetails.rdWebUrls.map((urlDetails) =>
            [
                urlDetails.url,
    ...
    const handleSort = (field: keyof RdWebUrl) => {
        if (!fileDetails) return;

        const order = sortField === field && sortOrder === "asc" ? "desc" : "asc";

        const sortedUrls = [...fileDetails.rdWebUrls].sort((a, b) => {
        ...
        setFileDetails({ ...fileDetails, rdWebUrls: sortedUrls });
        setSortField(field);
        setSortOrder(order);
    };

    const filteredUrls = fileDetails?.rdWebUrls.filter((urlDetails) =>
...
export default RdWebDetailsPage;
