...
interface FileDetails {
    filename: string;
    joomlaUrls: WordPressUrl[];
}

const JoomlaDetailsPage = ({ fileId }: { fileId: string }) => {
    ...
    useEffect(() => {
        const fetchFileDetails = async () => {
            try {
                const response = await fetch(`/api/joomlafiles/${fileId}`);
                if (!response.ok) {
                    throw new Error("Failed to fetch file details");
                }

                const data = await response.json();
                const joomlaUrls = data.file.joomlaUrls;

                // Fetch Moz API data for each URL
                const enrichedUrls = await Promise.all(
                    joomlaUrls.map(async (urlDetails: WordPressUrl) => {
                ...
                setFileDetails({ ...data.file, joomlaUrls: enrichedUrls });
            } catch (err) {
    ...
    const handleExportToCSV = () => {
        ...
        const csvRows = fileDetails.joomlaUrls.map((urlDetails) =>
            [
                urlDetails.url,
    ...
    const handleSort = (field: keyof WordPressUrl) => {
        ...
        const sortedUrls = [...fileDetails.joomlaUrls].sort((a, b) => {
        ...
        setFileDetails({ ...fileDetails, joomlaUrls: sortedUrls });
        setSortField(field);
        setSortOrder(order);
    };

    const filteredUrls = fileDetails?.joomlaUrls.filter((urlDetails) =>
...
export default JoomlaDetailsPage;
