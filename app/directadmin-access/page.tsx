...
const DirectAdminFilesPage = () => {
  const [files, setFiles] = useState<any[]>([]);

  useEffect(() => {
    const fetchFiles = async () => {
      try {
        const response = await fetch("/api/directadminfiles"); // Internal API route
        if (!response.ok) {
                ...
                {files.map((file, index) => (
                  <tr key={file._id}>
                    ...
                    <td className="border border-gray-300 px-4 py-2 text-center">
                      {file.directAdminUrls?.length || 0}
                    </td>
                    <td className="border border-gray-300 px-4 py-2 text-center">
                      <Link href={`/directadmin-access/${file._id}`}>
...
export default DirectAdminFilesPage;
