...
const COLLECTION_NAME = "pleskUrls"; // Replace with your collection name
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Fetch all files from the collection, including pleskUrls
    const files = await collection
      .find({}, { projection: { filename: 1, pleskUrls: 1 } }) // Fetch `filename`, `_id`, and `pleskUrls`
      .toArray();
