...
const COLLECTION_NAME = "directAdminUrls"; // Replace with your collection name
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Fetch all files from the collection, including directAdminUrls
    const files = await collection
      .find({}, { projection: { filename: 1, directAdminUrls: 1 } }) // Fetch `filename`, `_id`, and `directAdminUrls`
      .toArray();
